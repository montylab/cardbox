...
(function () {
	...
	var gameplayView = {
		...
		generateMainWord: function () {
			var word;
			while ((word = kids[Math.floor(Math.random() * kids.length)]).length < 4 || word.w.length > 8 && (word.d && word.d.length > 77)) {} //eslint-disable-line
		...
		mouseDown: function (e) {
			...
			if (this.tapped != -1 && (device.mobile() || device.tablet())) {
				sym = this.lettersOnBoard[this.tapped];
